...
outliers <- c('SC_Quiescence P5: C6->G0',  'SC_EMT P2: C3->G4', 'SC_Cell Cycle P6: C1->G3')
...
for (ii in 1:nrow(dist.mat) ){
  ...
  # Cell Cycle I
  cols <- colnames(delta.obj)[which(delta.obj$Mode == 'Cell Cycle I')]
  cols <- setdiff(colnames(delta.obj)[which(delta.obj$Mode == 'Cell Cycle I')], outlier)
  ...
  # EMT
  cols <- colnames(delta.obj)[which(delta.obj$Mode == 'EMT')]
  ...
  names(prob) <- c('Cell Cycle', 'Cell Cycle II', 'EMT', 'Quiescence')
...
simpson.df <- simpson.df[which(! simpson.df$Sample %in% c(paste0('CO_P', 1:6))),]
g <- simpson.df %>%
